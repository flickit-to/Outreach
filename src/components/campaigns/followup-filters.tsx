...
function matchesEngagement(status: string, filter: string): boolean {
  switch (filter) {
    case "opened": return ["opened"].includes(status);
    case "clicked": return ["clicked"].includes(status);
    case "delivered": return ["delivered"].includes(status);
    case "sent": return ["sent"].includes(status);
    case "bounced": return ["bounced"].includes(status);
...
export function FollowupFilters({
  ...
  const [selectedDays, setSelectedDays] = useState<Set<string>>(new Set()); // empty = all days

  // Unique days from sends
  const days = useMemo(() => {
  ...
  const filteredContacts = useMemo(() => {
    ...
    return result;
  }, [parentSends, selectedEngagement, selectedDays]);

  // Compute trigger string for the campaign
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <p className="text-sm font-medium">Send to contacts who:</p>
          ...
          {ENGAGEMENT_OPTIONS.map((opt) => {
            ...
            return (
              <button
                key={opt.key}
                type="button"
                onClick={() => toggleEngagement(opt.key)}
              ...
              >
                <div className={`h-3.5 w-3.5 rounded-sm border flex items-center justify-center ${
                  active ? "bg-current border-current" : "border-muted-foreground/30"
                }`}>
      ...
      <div className="p-3 rounded-md bg-muted/50 border">
        ...
        <p className="text-xs text-muted-foreground mt-1 italic">
          Contacts marked as Replied, Meeting Booked, or Closed are automatically excluded.
        </p>
      </div>
    </div>
  );
